...
SRC_URI="http://pub.brother.com/pub/com/bsc/linux/dlf/mfc235clpr-1.0.1-1.i386.rpm
	 http://pub.brother.com/pub/com/bsc/linux/dlf/mfc235ccupswrapper-1.0.1-1.i386.rpm"
...
src_install() {
	...
	dosbin "${WORKDIR}/usr/bin/brprintconf_mfc235c"

	mkdir -p ${D}/opt/Brother || die
	cp -r ${WORKDIR}/usr/local/Brother/* ${D}/opt/Brother/ || die

	mkdir -p ${D}/usr/libexec/cups/filter || die
	( cd ${D}/usr/libexec/cups/filter/ && ln -s ../../../../opt/Brother/Printer/mfc235c/lpd/filtermfc235c brlpdwrappermfc235c ) || die
	mkdir -p ${D}/usr/local || die
	( cd ${D}/usr/local && ln -s ../../opt/Brother Brother ) || die
	mkdir -p ${D}/usr/share/cups/model || die
	( cd ${D}/usr/share/cups/model && ln -s ../../../../opt/Brother/Printer/mfcj265w/cupswrapper/brmfc235c.ppd ) || die
}

pkg_postinst () {
	ewarn "You really wanna read this."
	elog "You need to use brprintconf_mfc235c to change printer options"
	...
	elog "		brprintconf_mfc235c -pt A4"
	elog "Set 'Fast Normal' quality:"
	elog "		brprintconf_mfc235c -reso 300x300dpi"
	elog
	elog "For more options just execute brprintconf_mfc235c as root"
	elog "You can check current settings in:"
	elog "		/opt/Brother/Printer/mfcj265w/inf/brmfc235crc"
}
