...
src_install() {
	...
	local tmp_home="${T}/fakehome"
	mkdir -p "${tmp_home}" || die "Could not create temporary HOME"
	...
	# Drop broken packet capture support; bundled wpcap.so expects libpcap.so.0.8
	rm -f \
		"${ED}"/opt/cxoffice/lib/wine/i386-unix/wpcap.so \
		"${ED}"/opt/cxoffice/lib/wine/x86_64-unix/wpcap.so \
		|| die "Could not remove broken pcap libraries"
