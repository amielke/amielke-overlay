...
SLOT="0"
KEYWORDS="-* ~amd64 ~x86"
IUSE="+capi +cups +gphoto2 +gstreamer +jpeg +lcms +mp3 +nls +openal +opencl +opengl +png +scanner +ssl +v4l +vulkan"
...
src_install() {
    ...
    rm -f README changelog.txt || die

    dodir /opt/cxoffice
    cp -a "${S}"/. "${ED}/opt/cxoffice/" || die
    ...
    sed -i -e 's/cxwhich konqueror/false &/' \
        "${ED}/opt/cxoffice/bin/locate_gui.sh" || die

    # Fake HOME for cxmenu
    ...
    if [[ -e ${uninstall_desktop[0]} ]]; then
        rm -f "${uninstall_desktop[@]}" || die
    fi

    rm -f \
        "${ED}"/opt/cxoffice/lib/wine/i386-unix/wpcap.so \
        "${ED}"/opt/cxoffice/lib/wine/x86_64-unix/wpcap.so \
        || die

    sed -i \
        -e "s:\"${ED}\".::" \
        -e "s:${ED}::" \
        "${ED}/opt/cxoffice/lib/perl/CXMenuXDG.pm" || die
    ...
    sed -i -e "s:${ED}::" "${desktop_files[@]}" || die
