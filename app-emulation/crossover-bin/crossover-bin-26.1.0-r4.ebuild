...
src_install() {
	...
	# Fix icon symlinks generated by cxmenu; they must not point into ${ED}
	if [[ -d ${ED}/usr/share/icons ]]; then
		while IFS= read -r -d '' icon_link; do
			local target
			target=$(readlink "${icon_link}") || die "Could not read symlink target: ${icon_link}"

			if [[ ${target} == "${ED}"* ]]; then
				ln -snrf "${target#${ED}}" "${icon_link}" \
					|| die "Could not rewrite icon symlink: ${icon_link}"
			fi
		done < <(find "${ED}/usr/share/icons" -type l -print0)
	fi
	...
	# Drop bundled packet capture shims; they require libpcap.so.0.8,
	# which is not provided by current Gentoo libpcap.
	rm -f \
